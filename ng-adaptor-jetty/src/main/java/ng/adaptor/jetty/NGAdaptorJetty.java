	...
	public class NGHandler extends Handler.Abstract {
		...
		private void doRequest( final Request jettyRequest, final Response jettyResponse, Callback callback ) throws IOException {
			...
			if( ngResponse instanceof NGResponseMultipart mp ) {
				...
				// FIXME:
				// Below is the old way we served streaming resources. Above is what's probably the proper way to serve streams.
				...
				// Also; double check that ng's InputStream is getting closed by Jetty once copied, otherwise keep the auto-closing from below.
				// Hugi 2025-06-17
				//
